...
class TestMicrostructureFromFile(unittest.TestCase):
    def test_from_file_creation(self):
        ...
        fname_readme = os.path.join(os.path.dirname(fname_existing_ms), "README.md")
        ...
        for creation_function in [
            ...
            for fname in [fname_readme, fname_wrong_ending]:
                with self.assertRaises(Exception):
                    creation_function(fname)
        for creation_function in [Microstructure.from_npy, Microstructure.load, mcrpy.load]:
...
class TestMicrostructureMultiphaseSinglephase(unittest.TestCase):
    ...
    ms_cases = [ms_binary_2d, ms_binary_3d, ms_threephase_2d, ms_threephase_3d]
    ms_binary_cases = [ms_binary_2d, ms_binary_3d]
