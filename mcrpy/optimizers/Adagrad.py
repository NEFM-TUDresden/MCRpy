...
class Adagrad(TFOptimizer):
    def __init__(self, max_iter: int = 100, callback: callable = None, learning_rate: float = 0.001, **kwargs):
        super().__init__(max_iter=max_iter, callback=callback)
        self.opt = tf.keras.optimizers.Adagrad(learning_rate=learning_rate)
