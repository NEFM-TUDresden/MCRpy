...
class DPB3D(MultiPhaseDescriptor3D):
    """Dual-Phase boundary density in 3D, normalized so it is a proper descriptor"""
    ...
    def make_multiphase_descriptor(n_phases: int = None, **kwargs) -> callable:
        ...
        tile_img = make_image_padder_3d(1, 1, 1)
        ...
        def periodic_variation(img: tf.Tensor) -> tf.Tensor:
            ...
            phase_boundaries = [compute_phase_boundaries_3d(img_tiled) for img_tiled in imgs_tiled]
            results_list = []
...
def register() -> None:
    descriptor_factory.register("DPB3D", DPB3D)
