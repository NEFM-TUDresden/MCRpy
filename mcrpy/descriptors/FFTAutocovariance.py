...
class FFTAutocovariance(PhaseDescriptor):
    """Autocovariance calculated by FFT"""
    ...
    def make_singlephase_descriptor(desired_shape_2d=(64, 64), limit_to: int = None, **kwargs) -> callable:
        ...
        def compute_descriptor(microstructure: tf.Tensor) -> tf.Tensor:
            ...
            real_coefficients = tf.cast(real_coefficients, tf.float64)
            vf = tf.reduce_max(real_coefficients, axis=None)
            real_coefficients = tf.signal.fftshift(real_coefficients) - vf * vf
            selected_components = tf.reshape(tf.boolean_mask(real_coefficients, mask), descriptor_shape)
    ...
    def visualize_subplot(
        ...
        if axis:
            ax.set_title(r"$\chi$" + f": l={mg_level}, p={n_phase}")
...
def register() -> None:
    descriptor_factory.register("FFTAutocovariance", FFTAutocovariance)
