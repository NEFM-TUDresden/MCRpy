...
tf.get_logger().setLevel(log.logging.ERROR)
...
class FFTCorrelations(PhaseDescriptor):
    ...
    def make_singlephase_descriptor(desired_shape_2d=(64, 64), limit_to: int = None, **kwargs) -> callable:
        ...
        def compute_descriptor(microstructure: tf.Tensor) -> tf.Tensor:
            ...
            real_coefficients_real = tf.math.real(real_coefficients)
            real_coefficients_real = tf.cast(real_coefficients_real, tf.float64)
            selected_components = tf.reshape(tf.boolean_mask(real_coefficients_real, mask), descriptor_shape)
