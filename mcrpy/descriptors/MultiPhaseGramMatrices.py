...
class MultiPhaseGramMatrices(MultiPhaseDescriptor):
    ...
    def make_multiphase_descriptor(
        ...
        **kwargs,
        ...
        mean_values = weights_data["mean value"].astype(np.float64)
        ...
        def make_conv2d_layer(weights: tf.Tensor, strides: tf.Tensor = [1, 1, 1, 1]) -> callable:
            ...
            if periodic:
                ...
                def conv2d_layer(ms_pre_conv):
                    ...
                    ms_post_conv = tf.nn.conv2d(ms_periodic, filters=weights, strides=strides, padding="VALID")
                    return ms_post_conv
                ...
                @tf.function
                def conv2d_layer(ms_pre_conv):
                    ms_post_conv = tf.nn.conv2d(ms_pre_conv, filters=weights, strides=strides, padding="SAME")
                    return ms_post_conv
        ...
        def make_encoding_layer() -> callable:
            ...
            extension_filters = tf.constant(np.zeros((1, 1, 1, 3), dtype=np.float64) + 255)
            ...
            if n_phases == 2:
                ...
                def encoding_layer(ms_in: tf.Tensor) -> tf.Tensor:
                    ...
                    ms_extended = tf.nn.conv2d(ms_p1, extension_filters, strides=(1, 1), padding="VALID")
                    ms_shifted = tf.nn.bias_add(ms_extended, subtraction_biases)
                    return ms_shifted
                ...
                @tf.function
                def encoding_layer(ms_in: tf.Tensor) -> tf.Tensor:
                    ms_shifted = tf.nn.bias_add(ms_in * 255, subtraction_biases)
                    return ms_shifted
                ...
                def encoding_layer(ms_in: tf.Tensor) -> tf.Tensor:
                    ...
                    ms_shifted = tf.nn.bias_add(ms_except_p0 * 255, subtraction_biases)
                    return ms_shifted
        ...
        def make_nl_layer(bias: tf.Tensor, nl_method: str = "relu") -> callable:
            ...
            if nl_method == "relu":
                ...
                @tf.function
                def nl_layer(ms_pre_bias):
                    ms_post_bias = tf.nn.leaky_relu(ms_pre_bias + bias, alpha=0.05)
                    return ms_post_bias
        ...
        def make_avgpool_layer(
            kernel_size: List[int] = [1, 2, 2, 1], strides: List[int] = [1, 2, 2, 1], padding: str = "VALID"
        ) -> callable:
        ...
        def make_gram_layer(weight: int = 1) -> callable:
            ...
            def l_gram(ms_pre_gram: tf.Tensor) -> tf.Tensor:
                ...
                ms_post_gram = compute_normalized_gram_matrix(ms_pre_gram, layer_area, out_channels)
            ...
            def l_gram_diag(ms_pre_gram: tf.Tensor) -> tf.Tensor:
                ms_post_gram = tf.linalg.tensor_diag_part(l_gram(ms_pre_gram))
                return ms_post_gram
        ...
        mg_c1 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[0])))(mg_enc)
        mg_n1 = tf.keras.layers.Lambda(make_nl_layer(weights[1], nl_method=nl_method))(mg_c1)
        mg_c2 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[2])))(mg_n1)
        mg_n2 = tf.keras.layers.Lambda(make_nl_layer(weights[3], nl_method=nl_method))(mg_c2)
        mg_a1 = tf.keras.layers.Lambda(make_avgpool_layer())(mg_n2)
        mg_c3 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[4])))(mg_a1)
        mg_n3 = tf.keras.layers.Lambda(make_nl_layer(weights[5], nl_method=nl_method))(mg_c3)
        mg_c4 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[6])))(mg_n3)
        mg_n4 = tf.keras.layers.Lambda(make_nl_layer(weights[7], nl_method=nl_method))(mg_c4)
        mg_a2 = tf.keras.layers.Lambda(make_avgpool_layer())(mg_n4)
        mg_c5 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[8])))(mg_a2)
        mg_n5 = tf.keras.layers.Lambda(make_nl_layer(weights[9], nl_method=nl_method))(mg_c5)
        mg_c6 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[10])))(mg_n5)
        mg_n6 = tf.keras.layers.Lambda(make_nl_layer(weights[11], nl_method=nl_method))(mg_c6)
        mg_c7 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[12])))(mg_n6)
        mg_n7 = tf.keras.layers.Lambda(make_nl_layer(weights[13], nl_method=nl_method))(mg_c7)
        mg_c8 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[14])))(mg_n7)
        mg_n8 = tf.keras.layers.Lambda(make_nl_layer(weights[15], nl_method=nl_method))(mg_c8)
        mg_a3 = tf.keras.layers.Lambda(make_avgpool_layer())(mg_n8)
        mg_c9 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[16])))(mg_a3)
        mg_n9 = tf.keras.layers.Lambda(make_nl_layer(weights[17], nl_method=nl_method))(mg_c9)
        mg_c10 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[18])))(mg_n9)
        mg_n10 = tf.keras.layers.Lambda(make_nl_layer(weights[19], nl_method=nl_method))(mg_c10)
        mg_c11 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[20])))(mg_n10)
        mg_n11 = tf.keras.layers.Lambda(make_nl_layer(weights[21], nl_method=nl_method))(mg_c11)
        mg_c12 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[22])))(mg_n11)
        mg_n12 = tf.keras.layers.Lambda(make_nl_layer(weights[23], nl_method=nl_method))(mg_c12)
        mg_a4 = tf.keras.layers.Lambda(make_avgpool_layer())(mg_n12)
        mg_c13 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[24])))(mg_a4)
        mg_n13 = tf.keras.layers.Lambda(make_nl_layer(weights[25], nl_method=nl_method))(mg_c13)
        mg_c14 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[26])))(mg_n13)
        mg_n14 = tf.keras.layers.Lambda(make_nl_layer(weights[27], nl_method=nl_method))(mg_c14)
        mg_c15 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[28])))(mg_n14)
        mg_n15 = tf.keras.layers.Lambda(make_nl_layer(weights[29], nl_method=nl_method))(mg_c15)
        mg_c16 = tf.keras.layers.Lambda(make_conv2d_layer(transpose(weights[30])))(mg_n15)
        mg_n16 = tf.keras.layers.Lambda(make_nl_layer(weights[31], nl_method=nl_method))(mg_c16)
        ...
        for layer_name, layer_weight in gram_weights.items():
            if layer_weight > 0:
                mg_gram = tf.keras.layers.Lambda(make_gram_layer(weight=layer_weight))(layer_name_to_layer[layer_name])
                mg_gram_flattened = tf.keras.layers.Lambda(lambda x: tf.reshape(x, (1, -1)))(mg_gram)
                gram_layers.append(mg_gram_flattened)
        outputs = tf.keras.layers.Concatenate()(gram_layers) if len(gram_layers) > 1 else gram_layers[0]
