...
class SurfaceCorrelations(PhaseDescriptor):
    """2-point auto-correlation of surface indicator function, calculated by FFT """

    is_differentiable = True

    @staticmethod
    def make_singlephase_descriptor(
        desired_shape_2d=(64, 64), limit_to: int = None, periodic: bool = True, **kwargs
    ) -> callable:
        ...
        np_filters = np.zeros((3, 3, 1, 1), dtype=np.float64)
        np_filters[1, :, :, :] = 1
        np_filters[:, 1, :, :] = 1
        np_filters[1, 1, :, :] = -4
        tf_filters = tf.constant(np_filters, dtype=tf.float64)
        surface_convolution = make_conv2d_layer(tf_filters)

        @tf.function
        def detect_surfaces(microstructure: tf.Tensor) -> tf.Tensor:
            signed_indicator = surface_convolution(microstructure)
            # unsigned_indicator = tf.math.abs(signed_indicator)
            unsigned_indicator = signed_indicator * signed_indicator
            return unsigned_indicator

        @tf.function
        def compute_descriptor(microstructure: tf.Tensor) -> tf.Tensor:
            # microstructure = tf.reshape(microstructure, desired_shape_2d)
            surfaces = detect_surfaces(microstructure)
            surfaces = tf.reshape(surfaces, desired_shape_2d)
            ms_fourier = tf.signal.fft2d(tf.cast(surfaces, tf.complex128))
...
def register() -> None:
    descriptor_factory.register("SurfaceCorrelations", SurfaceCorrelations)
