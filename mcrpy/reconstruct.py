...
def main(args):
    ...
    convergence_data, last_frame = reconstruct(
        descriptor_dict, desired_shape, settings=settings, initial_microstructure=initial_microstructure
    )
...
def prepare_reconstruction(args):
    ...
    desired_shape = (
        (args.extent_x, args.extent_y, args.extent_z)
        if args.extent_z not in {None, 0, 1}
        else (args.extent_x, args.extent_y)
    )
    del args.extent_x
    del args.extent_y
    del args.extent_z
...
def setup_settings(descriptor_dict, desired_shape, settings, initial_microstructure):
    ...
    if any(s_i in {0, 1, None} for s_i in desired_shape):
        raise ValueError(
            f"Provided desired_shape {desired_shape} should not contain " + "entries that are 0, 1 or None."
        )
...
def cli_main():
    ...
    parser.add_argument(
        "--descriptor_filename", type=str, help="File of descriptor that to reconstruct from", default=""
    )
    parser.add_argument(
