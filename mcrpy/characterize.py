...
def make_descriptor_functions(
    microstructure_shape: Tuple[int], settings: CharacterizationSettings
) -> Dict[str, callable]:
    """Create descriptor functions given the microstructure shape and some settings."""
    if len(microstructure_shape) == 2:
        raw_descriptors = {
            descriptor_type: descriptor_factory.create(descriptor_type, arguments=vars(settings))
...
@log.log_this
def run_characterization(ms: Microstructure, descriptors: Dict[str, callable]) -> Dict[str, np.ndarray]:
    """Run the characterization for all descriptors."""
    return {
        descriptor_type: convert(descriptor_function(ms))
...
def save_characterization(
    characterization: Dict[str, Union[np.ndarray, Tuple[np.ndarray]]], info_file_additive: str, target_folder: str
):
    """Save characterization to a pickle file."""
    char_file = os.path.join(target_folder, f"{info_file_additive}characterization.pickle")
...
def characterize(
    microstructures: Union[Microstructure, List[Microstructure]], settings: CharacterizationSettings = None
...
def cli_main():
    ...
    parser.add_argument(
        "--microstructure_filenames", nargs="+", type=str, help="File or files of MS to characterize", default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--limit_to", type=int, help="Limit in pixels to which to limit the characterization metrics.", default=8
    )
    parser.add_argument(
        "--threshold_steepness",
        type=float,
