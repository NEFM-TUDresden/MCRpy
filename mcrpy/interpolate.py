...
def read_descriptors(args: argparse.Namespace) -> List[Dict[str, np.ndarray]]:
    ...
    for ms_char in ms_chars:
        if keys != ms_char.keys():
            raise ValueError("Key Mismatch! Make sure all characterization files " + "contain the same descriptors.")
    return ms_chars


@log.log_this
def interpolate(ms_char_from: Dict[str, np.ndarray], ms_char_to: Dict[str, np.ndarray], n_steps: int):
    """Interpolate between two diferent microstructure characterizations in the descriptor space."""
    if n_steps <= 2:
        raise ValueError(f"Number of steps is {n_steps}, but should be > 2 because steps include start and end.")
                ...
                if merged:
                    ...
                    for ii in range(len(d_from)):
                        ...
                        if d_to[ii].shape != og_shape:
                            raise ValueError(
                                "Shape mismatch! Make sure the descriptors to " + "interpolate have the same shape."
                            )
