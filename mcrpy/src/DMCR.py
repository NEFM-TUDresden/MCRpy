...
class DMCR:
    def __init__(
        ...
        **kwargs,
        ...
        self.kwargs = kwargs
        self.convergence_data = {"raw_data": [], "line_data": [], "scatter_data": []}
        self._pbar = None
        self._pbar_last_iter = 0

        if not self.use_multiphase:
            self.phase_sum_multiplier = 0.0

        # load modules
        loader.load_plugins([f"mcrpy.descriptors.{descriptor_type}" for descriptor_type in self.descriptor_types])
        ...
        orientation_descriptors = [
            issubclass(descriptor_factory.descriptor_classes[descriptor_type], OrientationDescriptor)
    ...
    def setup_descriptor(self):
        ...
        if self.non_cubic_3d:
            ...
            for desired_shape_2d in desired_shapes_2d:
                ...
                self.descriptors.append(
                    [
                        descriptor_factory.create(
                            descriptor_type, arguments=descriptor_kwargs, assert_differentiable=self.is_gradient_based
                        )
                        for descriptor_type in self.descriptor_types
                    ]
                )
        ...
        desired_descriptor_shapes = [
            desired_value[0].shape[2:] if self.anisotropic else desired_value.shape[2:]
            for desired_value in self.desired_descriptor
        ]
        self.descriptor_comparisons = [
            descriptor_factory.get_class(descriptor_type).make_comparison(
                desired_descriptor_shape=desired_descriptor_shape, limit_to=self.limit_to, **self.kwargs
            )
            for descriptor_type, desired_descriptor_shape in zip(self.descriptor_types, desired_descriptor_shapes)
        ]
    ...
    def validate_n_phases(self):
        for desired_value, d_is_multiphase in zip(self.desired_descriptor, self.descriptor_is_multiphase):
            if d_is_multiphase:
                continue
            if self.anisotropic:
    ...
    def reconstruct(self, desired_descriptor, desired_shape):
        ...
        # add last state
        self.reconstruction_callback(last_iter + 1, self.opt.current_loss, self.ms, force_save=True, safe_mode=True)
    ...
    def setup_optimization(self, desired_shape, mg_levels, previous_solution, mg_level):
        ...
        n_digits = len(str(self.max_iter))
        header = f"\t{'Time':>8}  " f"{'Iter':>{n_digits*2+1}}  " f"{'Loss':>12}"
    ...
    def _determine_mg_levels(self, desired_descriptor, desired_shape):
        if self.use_multigrid_reconstruction:
            ...
            mg_levels_d = (
                min(d_des[0].shape[1] for d_des in desired_descriptor)
                if self.anisotropic
                else min(d_des.shape[1] for d_des in desired_descriptor)
            )
    ...
    def _create_optimizer(self):
        ...
        self.opt.set_call_loss(
            loss_computation.make_call_loss(
                self.loss,
                self.ms,
        ...
        if optimizer_factory.optimizer_classes[self.optimizer_type].is_vf_based:
            if self.volume_fractions is None:
                raise ValueError(
                    f"The chosen optimizer {self.optimizer_type} "
