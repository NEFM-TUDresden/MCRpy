...
optimizer_choices = [
    d[:-3]
    for d in os.listdir(os.path.join(os.path.dirname(os.path.dirname(__file__)), "optimizers"))
    if d.endswith(".py") and d not in {"Optimizer.py", "TFOptimizer.py", "SPOptimizer.py", "__init__.py"}
]


def register(optimizer_type: str, creator_fn: Callable[..., Optimizer]) -> None:
...
def create(optimizer_type: str, arguments: Dict[str, Any]) -> Optimizer:
    try:
