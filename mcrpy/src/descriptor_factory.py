...
descriptor_choices = [
    d[:-3]
    for d in os.listdir(os.path.join(os.path.dirname(os.path.dirname(__file__)), "descriptors"))
...
def create(descriptor_type: str, arguments: Dict[str, Any], assert_differentiable: bool = False) -> callable:
    ...
    assert issubclass(descriptor_classes[descriptor_type], PhaseDescriptor) or issubclass(
        descriptor_classes[descriptor_type], OrientationDescriptor
...
def permute(
    ...
    if shape_3d[0] == shape_3d[1] == shape_3d[2]:
        ...
        for spatial_dim, desired_shape_2d in enumerate(desired_shapes_2d):
            ...
            descriptor_functions.append(create(descriptor_type, arguments=arguments))

    def get_slice_descriptor(ms: Microstructure, spatial_dim: int, n_slice_outer: int) -> np.ndarray:
    ...
    if mode.lower() == "average":
        ...
        def permutation_loop(ms: Microstructure):
            ...
            for spatial_dim in range(3):
                ...
                descriptors.append(get_slice_descriptor(ms, spatial_dim, n_slice_outer))
            if isotropic:
