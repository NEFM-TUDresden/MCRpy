...
def create_target_folder(args: argparse.Namespace) -> str:
    ...
    if args.data_folder is None:
        args.data_folder = f"DefaultFolder {time.asctime()}".replace(" ", "-").replace(":", "-")
...
def copy_ms_to_target(target_folder: str, args: argparse.Namespace) -> str:
    ...
    for microstructure_filename in args.microstructure_filenames:
        microstructure_copyto = os.path.join(target_folder, os.path.split(microstructure_filename)[1])
...
def load(filename: str, use_multiphase: bool = False) -> Union[np.ndarray, Dict]:
    if filename.endswith(".npy"):
        ...
        raise ValueError(
            "Can only load microstructures as npy- or pickle-files and descriptors as pickle-files. "
