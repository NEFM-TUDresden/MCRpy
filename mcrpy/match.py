...
def main(args: argparse.Namespace):
    ...
    characterization, convergence_data, last_frame = match(
        microstructure, add_dimension=add_dimension, initial_microstructure=initial_microstructure, settings=settings
    )
...
def prepare_saving(settings, microstructure_filename):
    info_file_additive = (
        ...
        + "_"
        + (f"{settings.information}_" if settings.information is not None else "")
    )
    information_additives = "" if settings.information is None else f"_{settings.information}"
...
def setup_reconstruction(microstructure, add_dimension, settings):
    ...
    if add_dimension is not None:
        if microstructure.is_3D:
            raise ValueError("Provided settings for add_dimension, but microstructure is " + "already 3D.")
...
def cli_main():
    parser = argparse.ArgumentParser(
        description="Match a given microstructure by characterising and then reconstructing it."
    )
    parser.add_argument(
        "--microstructure_filenames",
        nargs="+",
        type=str,
        help="File of MS that shall be matched",
    ...
    parser.add_argument(
        "--data_folder", type=str, help="Results folder. If None, default with timestamp.", default="results"
    )
    parser.add_argument(
        "--descriptor_types",
        nargs="+",
    ...
    parser.add_argument(
        "--optimizer_type", type=str, help="Optimizer type defined in optimizer plugins.", default="LBFGSB"
    )
    ...
    parser.add_argument(
        "--limit_to", type=int, help="Limit in pixels to which to limit the characterisation metrics.", default=16
    )
    parser.add_argument(
        "--threshold_steepness",
        type=float,
    ...
    parser.add_argument(
        "--phase_sum_multiplier", type=float, help="Penalty weight for phase sum in loss", default=1000.0
    )
    ...
    parser.add_argument(
        "--batch_size",
        type=float,
        help="Batch size for greedy optimization in 3D, ie how many slices are considered per step",
        default=1,
    )
