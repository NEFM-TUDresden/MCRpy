    ...
    def make_loss(
        ...
        **kwargs,
        ...
        if dropout_rate is not None:
            num_samples = tf.constant(
                sum(np.prod(desired_descriptor.shape) for desired_descriptor in desired_descriptor_list)
                * ((mg_level + 1) / desired_descriptor_list[0].shape[1]),
                dtype=tf.int32,
            )

            @tf.function
            def draw_kept_components() -> tf.Tensor:
                component_mask = tf.cast(
                    tf.random.categorical(tf.math.log([[dropout_rate, 1 - dropout_rate]]), num_samples, seed=1),
                    tf.float64,
                )
        ...
        if desired_descriptor_list[0].shape[0] > n_phases:
            logging.warning(
                "desired_descriptor has multiphase, but multiphase is switched off for reconstruction. Using phase 1"
            )
            phases_desired = [1]
        ...
        def compute_energy(x):
                        ...
                        descriptor_differences.append(
                            tf.reshape(
                                ...
                                * descriptor_comparison(
                                    ...
                                    desired_descriptor[phase_desired, n_mg_level + mg_level],
                                ),
                                [1, -1],
                            )
                        )
                try:
                    weighted_descriptor_difference = (
                        tf.concat(descriptor_differences, axis=0)
                        if len(descriptor_differences) > 1
                        else descriptor_differences[0]
                    )
            ...
            difference = (
                ...
                if len(flattened_difference_list) > 1
                else flattened_difference_list[0]
            )
            if dropout_rate is None:
                ...
                difference_mb = tf.math.multiply(difference, tf.reshape(component_mask, tf.shape(difference)))
        ...
        def compute_oor_penalty(x):
            out_of_range = tf.math.square(tf.nn.relu(x - 1) + tf.nn.relu(0 - x))
